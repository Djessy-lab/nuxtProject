      ...
      <div class="rounded-md shadow-lg w-[50%] h-[70vh] max-lg:w-full max-lg:h-full">
        <div class="">
          ...
          <div class="flex justify-center p-4 mx-auto mt-10 border rounded-md h-80 w-96">
            <p>
              <input v-model="name" @input="report.name = $event.target.value" type="text" class="border rounded-md w-full h-10 p-2" placeholder="Nom" />
              <input v-model="date" @input="report.date = $event.target.value" type="date" class="border rounded-md w-full h-10 mt-4 p-2" placeholder="Date" />
          ...
          <div class="flex justify-center mt-10">
            <Button text="Ajouter" variant="3" @click="submitReport" />
          </div>
        </div>
      </div>
    </div>
...
`
  ...
  methods: {
    submitReport() {
      this.$emit('newReport', this.report)
    },
  },
}
</script>
